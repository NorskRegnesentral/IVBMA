...
helper <- function(j)
{
    ...
    w_X <- c(5:23)
    w_Z <- c(24:42)
    w_W <- c(51:55,45:50,52:p)

    p.W = length(w_W) + 1
    p.X = length(w_X)
    
    pi.M <- c(rep(0.5,5), rep(1, p.W - 5), rep(0.5, p.X))
    Theories <- c(rep(1,5), rep(2,6),rep(3,p.W - 12),4,5,6,7,7,8,8,8,8,9,10,11,11,11,12,12,12,12,12,13)
    m = max(Theories)
    Theories_prob <- c(0.5, rep(1,3), rep(0.5, m - 3))
    which_intercept = 4
